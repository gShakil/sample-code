...
namespace __ENF::nsconnector_v2
{
	...
	size_t ftdideviceConnector::send(std::vector<unsigned char> vdata, int param /*= 0*/)
	{
		if (isconnect())
		{
			...
			printdebug(std::format("FT_Write ret: {} [{}]", stat, vec2string(vdata)));
			assert(stat == FT_OK);
	...
	std::int64_t ftdideviceConnector::recv(unsigned char* buffer, size_t buflen)
	{
		...
		printdebug(std::format("recv - queue status: {}, rxQueue: {}", stat, rxQueue));
		...
		if (rxQueue > 0)
		{
			...
			if (read > 0)
				printdebug(std::format("FT_Read ret: {} toread: {} rdsize: {} [{}]"
					, stat, toRead, read, vec2string(std::vector<unsigned char>(buffer, buffer + read))));
			// 			else
