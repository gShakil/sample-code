...
CStringA CDynDialogItemEx::GetClassNameByType(DLGITEMTEMPLATECONTROLS TypeControl)
{
	switch(TypeControl) {
	case BUTTON:
		return "BUTTON";
...
BOOL CDynDialogItemEx::CreateEx(CWnd *pParent)
{
	...
	if (m_eTypeControl == NOCONTROL) { //It will probably be an OCX...
		...
		bRet = ((CWnd*)m_pData)->CreateEx(m_dwStyleEx, CString(m_strClassName), CString(m_strCaption), m_dwStyle, m_Rect, pParent, m_ControlID);
	}
	else  {
		bRet = CWnd::CreateEx(m_dwStyleEx, CString(m_strClassName), CString(m_strCaption), m_dwStyle, m_Rect, pParent, m_ControlID);
	}
...
BOOL CDynDialogItemEx::SetWindowPos(CWnd *pParent)
{
	...
	if (m_eTypeControl == NOCONTROL) {
		CString strClassName(m_strClassName);
		BSTR bstrLicKey = GetRuntimeLicense(strClassName);
		bRet = CreateControl(CString(m_strClassName), CString(m_strCaption), m_dwStyle, rect, pParent, m_ControlID, NULL, FALSE, bstrLicKey);
		if (bstrLicKey != NULL) {
