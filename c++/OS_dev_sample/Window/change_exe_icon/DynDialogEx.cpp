...
INT_PTR CDynDialogEx::DoModal()
{
	...
	int cWC = MultiByteToWideChar(CP_ACP, 0, CStringA(LogFont.lfFaceName), -1, NULL, 0);
	int nFontNameLen = cWC + 1;
	WCHAR *szFontName = new WCHAR[nFontNameLen];
	// Copy the string
	MultiByteToWideChar(CP_ACP, 0, CStringA(LogFont.lfFaceName), -1, (LPWSTR) szFontName, cWC);
	szFontName[cWC] = 0;
